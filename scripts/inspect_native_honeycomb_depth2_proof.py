...
EXPECTED = {
    ...
    "supervisor": {
        "agent_role": "gpt_luna_supervisor",
        "provider": "openai",
        "model": "gpt-5.6-luna",
        "effort": "medium",
        "depth": 1,
    ...
    "worker": {
        "agent_role": "ollama_worker",
        "provider": "agent_workbench_ollama",
...
def session_summary(path: Path) -> dict[str, Any]:
    ...
    efforts = sorted({context.get("effort") for context in contexts if context.get("effort")})
    versions = sorted({context.get("multi_agent_version") for context in contexts if context.get("multi_agent_version")})
    shell_calls = sum(
        1
        for event in events
...
def verdict(root_path: Path, supervisor_path: Path, worker_path: Path) -> dict[str, Any]:
    summaries = {
        "root": session_summary(root_path),
        "supervisor": session_summary(supervisor_path),
        "worker": session_summary(worker_path),
    }
    errors: list[str] = []
    for name, summary in summaries.items():
        validate_role(name, summary, errors)
    root = summaries["root"]
    supervisor = summaries["supervisor"]
    ...
    expected_spawns = ((root, "gpt_luna_supervisor"), (supervisor, "ollama_worker"))
    for parent, agent_type in expected_spawns:
...
def main() -> int:
    ...
    if missing:
        result = {
            "schema_version": 1,
            "transport": "codex_native_handoff",
            "recursive_protocol_accepted_candidate": False,
            "interactive_ui_usable_candidate": False,
            "economics_usable": False,
            "errors": [f"missing rollout: {path.name}" for path in missing],
        }
    else:
