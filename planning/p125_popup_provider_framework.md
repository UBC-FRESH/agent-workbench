...
1. **Right-size before requesting.** The smallest allocation that fits the
   ...
   Right-sizing applies to **every** billed dimension, not just GPUs — and the
   dominant one is scheduler-specific. On Sockeye (`MAX_TRES`, `Mem=5.00G`,
   `gres/gpu=6.0`) a 120G memory request bills 600 while 4 GPUs bill 24, so
   memory governs cost and GPU right-sizing alone changes nothing. Read the
   target's `TRESBillingWeights` and `PriorityFlags` before assuming which
   dimension to shrink. See `notes/clusters/sockeye.md`.
