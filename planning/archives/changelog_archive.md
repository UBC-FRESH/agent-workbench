...
Complete historical changelog for this repository, oldest first. The most
recent entries live in `CHANGE_LOG.md`.

Repaired 2026-07-30: the 2026-07-23 migration preserved only 30 of these
entries. This file now contains all 370 historical entries,
reconstructed from the pre-migration changelog and the previous archive.
...
## 2026-07-22 - Activated P120 CPU document scraper parallel lane

- Created parent issue #749 and branch
  `feature/p120-cpu-document-scraper-lane` after explicit maintainer approval
  to operate P120 in parallel with open P109 and P119 work.
- Created child issues #750 (extraction contract and playbook), #751
  (resumable coordinator), and #752 (registered benchmark and route decision).
  The parent issue records the complete phase scope, task links, acceptance
  criteria, verification, and closeout requirements.
- Quality: not yet measured against a live CPU model endpoint. P120.1 is the
  active task; no benchmark or production-readiness result is claimed.
- Protocol: the phase/branch/child-issue hierarchy, roadmap, planning note,
  and change log now identify the same approved parallel lane. Live endpoint
  routes, credentials, model files, raw texts, and raw logs remain excluded.
- Economics: not yet measured. A future registered benchmark must record
  validated throughput, latency, retries, fallback use, and resource evidence
  before any CPU-versus-GPU claim.

## 2026-07-22 - Prepared CPU-only Ollama backend for P120

- Added a reversible Ollama systemd drop-in that disables CUDA, ROCm, and Vulkan
  GPU visibility, then restarted only the Ollama service. The separate vLLM
  service was not restarted or reconfigured.
- Verified a bounded `llama3.1:latest` OpenAI-compatible JSON-mode completion
  at `100% CPU` placement; NVIDIA reported no Ollama `llama-server` compute
  process after the request.
- Recorded the generic CPU-only service configuration and Ollama profile in the
  P120 playbook. The existing Cloudflare Tunnel supports multiple public
  hostname-to-local-service routes, so a separate tunnel is not required for a
  future Ollama hostname; no Cloudflare routing or DNS mutation was made.
- Quality: protocol compatibility only; no P108 extraction benchmark or model
  quality verdict.
- Protocol: CPU isolation is verified locally; future external exposure needs a
  separate hostname, same-tunnel ingress/DNS route, and appropriate access
  policy.
- Economics: not measured; no CPU-versus-GPU claim.

## 2026-07-22 - Staged same-tunnel hostname migration safely blocked on Access

- Created same-tunnel DNS routes for `fresh01-vllm.01101.dev` and
  `fresh01-ollama.01101.dev` without changing the legacy
  `fresh01x.01101.dev` ingress rule or its vLLM origin.
- A bounded external probe showed the new vLLM hostname reached the origin with
  HTTP `200` rather than the legacy hostname's Cloudflare Access redirect. The
  new hostnames therefore do not inherit the legacy Access protection.
- Removed the two newly added ingress rules immediately while retaining their
  DNS records. Both local origins remain healthy; the new vLLM hostname now
  returns the terminal tunnel `404`, while legacy `fresh01x` remains unchanged
  and returns its normal Access `302` response.
- Quality: no extraction benchmark was run.
- Protocol: no unprotected external service is left active. Activating either
  new hostname now requires a dedicated Cloudflare Access application/policy,
  followed by a re-addition and validation of its ingress rule.
- Economics: not measured.

## 2026-07-22 - Activated protected same-tunnel vLLM and Ollama hostnames

- After Cloudflare Access policies were created for the staged hostnames,
  re-enabled same-tunnel ingress rules for `fresh01-vllm.01101.dev` to the
  local vLLM origin and `fresh01-ollama.01101.dev` to the local CPU-only Ollama
  origin.
- Verified that both new hostnames return Cloudflare Access redirects rather
  than unauthenticated origin responses. The legacy `fresh01x.01101.dev` route
  remains unchanged and returns its existing Access redirect.
- Quality: external route protection and origin connectivity are verified; no
  P108 extraction benchmark or model-quality verdict has been produced.
- Protocol: the two new hostnames share the existing tunnel, remain Access
  protected, and do not require a new tunnel or registrar update. The legacy
  vLLM route remains available during the staged migration.
- Economics: not measured.
