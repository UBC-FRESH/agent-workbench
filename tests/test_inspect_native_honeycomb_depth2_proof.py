        ...
        events.append(
            {
                ...
                "payload": {
                    ...
                    "arguments": json.dumps(
                        {"agent_type": spawn_role, "fork_context": False, "message": "bounded task"}
                    ),
                },
            }
        )
    for index in range(turns):
        events.extend(
            [
                ...
                {
                    "type": "event_msg",
                    "payload": {"type": "task_complete", "turn_id": f"turn-{index}", "last_agent_message": "ok"},
                },
            ]
        )
    return events
...
def test_accepts_role_bound_recursive_ui_chain(tmp_path: Path) -> None:
    ...
    write_jsonl(
        supervisor,
        session("supervisor", "root", "gpt_luna_supervisor", "openai", "gpt-5.6-luna", "medium", 1, "ollama_worker"),
    )
    write_jsonl(
        worker,
        session("worker", "supervisor", "ollama_worker", "agent_workbench_ollama", "qwen3.6:35b-a3b-bf16", "low", 2, turns=3),
    )
