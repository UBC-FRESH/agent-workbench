...
def test_request_preserves_normal_repair_and_advisory_boundary() -> None:
    request = build_supervisor_review_request(delta(), ticket_boundary=BOUNDARY)
    ...
    assert validate_delta_review_packet(packet(), delta=delta(), ticket_boundary=BOUNDARY).ok
...
def test_constructive_nudge_requires_all_evidence_and_boundary_fields() -> None:
    ...
    assert validate_delta_review_packet(value, delta=delta(), ticket_boundary=BOUNDARY).ok
...
def test_rejects_uncited_or_inappropriate_recommendations_and_freeform_nudges() -> None:
    ...
    result = validate_delta_review_packet(value, delta=delta(), ticket_boundary=BOUNDARY)
...
def test_rejects_empty_delta_and_nudge_boundary_expansion() -> None:
    ...
    result = validate_delta_review_packet(value, delta=empty, ticket_boundary=BOUNDARY)
...
def test_rejects_unsanitized_delta_fields_before_a_review_request() -> None:
    ...
    try:
        build_supervisor_review_request(unsafe, ticket_boundary=BOUNDARY)
...
def test_accepts_directive_deviation_escalation_and_terminal_review() -> None:
    ...
    assert validate_delta_review_packet(deviation, delta=delta(), ticket_boundary=BOUNDARY).ok
    assert validate_delta_review_packet(terminal, delta=delta(), ticket_boundary=BOUNDARY).ok
...
def test_malformed_delta_is_rejected_without_raising() -> None:
    ...
    result = validate_delta_review_packet(packet(), delta=malformed, ticket_boundary=BOUNDARY)
